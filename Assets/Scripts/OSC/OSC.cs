...
namespace UnityOSC
{
    ...
    public class UDPPacketIO
    {
        ...
        public bool IsOpen() => socketsOpen;
        ...
        public UDPPacket ReceivePacket(byte[] buffer)
        {
            if (!IsOpen())
                Open();
            if (!IsOpen())
                return new UDPPacket() { length = 0, address = "" };
            ...
            return new UDPPacket() { length = count, address = iep.ToString() };
        }
    ...
    public class OscMessage
    {
        ...
        /// <summary>
        /// The ip of the phone who send the message.
        /// </summary>
        public string ip;
    ...
    {
        [SerializeField] private int serverPort = 6969;
        [SerializeField] private string serverIP = "127.0.0.1";
        [SerializeField] private int clientPort = 6161;
        ...
        void Awake()
        {
            OscPacketIO = new UDPPacketIO(serverIP, clientPort, serverPort);
        ...
        public static OscMessage StringToOscMessage(string message)
        {
            ...
            IEnumerator sE = message.Split(' ').GetEnumerator();
            if (sE.MoveNext())
        ...
        public static ArrayList PacketToOscMessages(byte[] packet, UDPPacketIO.UDPPacket packetinfo)
        {
            ArrayList messages = new ArrayList();
            ExtractMessages(messages, packet, 0, packetinfo.length, packetinfo.address);
            return messages;
        }
        ...
        private static int ExtractMessages(ArrayList messages, byte[] packet, int start, int length, string address)
        {
            int index = start;
            switch ((char)packet[start])
            {
                case '/':
                    index = ExtractMessage(messages, packet, index, length, address);
                    break;
                case '#':
        ...
        private static int ExtractMessage(ArrayList messages, byte[] packet, int start, int length, string address)
        {
            OscMessage oscM = new OscMessage
            {
                address = ExtractString(packet, start, length),
                ip = address
            };
